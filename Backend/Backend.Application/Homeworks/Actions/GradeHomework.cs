    ...
    public async Task<StudentHomeworkDto> Handle(GradeStudentHomework request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            await _unitOfWork.NotificationRepository.AddNotificationAsync(new Notification
            {
                ...
                Message = $"You received a grade of {request.grade} for homeword #{request.HomeworkId}.",
                Type = NotificationType.Grade,
                SentByEmail = await _mailService.SendSimpleEmailAsync(student.ParentEmail, "New Grade Assigned", $"You received a grade of {request.grade} for homeword #{request.HomeworkId}.") // Optional fallback
            });
